...
**easystats** refers to 10 packages. Although each of them has a specific scope, they are meant to be working together. They are actually used by each-other, forming layers of bricks that aim at creating a space in which doing stats with R is easy.

Let's see this structure. First, at the bottom, we have the foundation upon which the ecosystem is built:

- [**insight**](https://github.com/easystats/insight): 🔮 This package serves as the foundation of the ecosystem as it allows manipulating objects from different R packages. It is very useful for developers, helping them to make their own packages flexible.
