...
At any rate, since the pandemic did not affect GitHub in any way, our free and open-source fanatic souls have made sure that this project keeps progressing, sometimes in leaps and bounds, while other times slow as the white cloud in the sky.
...
**easystats** project, which started out as a single R package, now comprises a whopping 10 packages! These modular packages, each with its own, unique purview, connect with each other to form a unifying, consistent, and most importantly, **easy-to-use** framework for statistical modeling, visualization, and reporting in R programming language! 🕸️
...
- [**effectsize**](https://easystats.github.io/effectsize): 🐉 A one-stop solution for computing pretty much every effect size known to the mankind, well, almost every. 😅
...
- [**report**](https://easystats.github.io/report): 📜 🎉 This packages provides an automated way to create text reports detailing results from statistical analyses.
...
To our utmost delight, these packages have already been downloaded **over 6 million times**! 🤯
...
Not only have the packages gained popularity among users, but also fellow R developers have started to rely on *easystats* packages for their own project.
...
* All of us contribute to this project in our free time and have surprisingly never met each other. This has also taught us the value, fruitfulness, and the sheer joy of online global collaboration, although we are well aware that all of us live in developed countries and enjoy the privilege of good internet connectivity.

* Another thing we learned is that our laptops look prettier with *easystats* package stickers. 😊

```{r, echo=FALSE}
knitr::include_graphics("assets/stickers.jpg")
```

If you also want to make your laptops pretties, [get in touch](https://twitter.com/easystats4u/status/1461674060283269123?s=20)!
